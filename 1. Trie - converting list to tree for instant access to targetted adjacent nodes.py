...
""" 
"""
Approach1: DFS ~ O(M*N * numWords * 3^(longestWordLength)) where M,N is board dimension, and Lmax is length of longest word
For every word, iterate through the board and do dfs if char matches.
"""

"""
Approach2: DFS + Trie ~ O(MN * numPaths * 3^(longestWordLength)) #here the worst case runtime is the same (assuming similar words like "aaaaab" "aaaaac", etc.)
...
Idea from: https://www.youtube.com/watch?v=EmvsBM7o-5k&ab_channel=TECHDOSE
"""
            ...
            if boardLetter in curNode.letterToChild:
                ...
                for dx,dy in [(-1,0),(1,0),(0,-1),(0,1)]:
                    ...
                    if 0 <= adjX < rows and 0 <= adjY < cols:
                        dfs(adjX,adjY,nextNode) #comeback1 I should also check nextNode to see if there's match to reduce recursive call stack size
            # no match with board or no more letters along this path
            # revert back the letter modified by dfs
...
def main():
    ...
    board, words = [["o","a","a","n"],["e","t","a","e"],["i","h","k","r"],["i","f","l","v"]], ["oath","pea","eat","rain"] #["eat","oath"]
    board, words = [["a","b"],["c","d"]], ["abcb"] #[]
    board, words = [["a"]], ["ab"] #[]
    board, words = [["a","a"]], ["a"]
    board, words = [["a","b"],["a","a"]], ["aba","baa","bab","aaab","aaa","aaaa","aaba"] #["aba","aaa","aaab","baa","aaba"]
    ...
    if testingOneInput:
        #Test single input
        print("Input: board={}, words={}".format(board, words))
        ...
        solution = Solution()
        for input in inputs:
            board, words = input[0], input[1]
            output = solution.findWords(board, words)
            print("Input: board={0}, words={1}, Output: {2}".format(board, words, output))
